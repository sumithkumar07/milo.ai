                  ...
                  {message.isStreaming && message.content === '' && (
                   ...
                   {message.ragSources && message.ragSources.length > 0 && (
                     <div className="mb-3 flex flex-wrap gap-1.5">
                       <span className="text-[10px] text-on-surface-variant/60 flex items-center gap-1 mr-1">
                         <FileText className="w-3 h-3" /> Sources:
                       </span>
                       {[...new Map(message.ragSources.map(s => [s.name, s])).values()].map((source, i) => (
                         <span key={i} className="px-2 py-0.5 bg-tertiary/10 border border-tertiary/20 rounded-full text-[10px] text-tertiary">
                           {source.name}
                         </span>
                       ))}
                     </div>
